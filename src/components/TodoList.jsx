...
const TODO_LIST_API_URL = "http://localhost:3002";
class TodoList extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`${TODO_LIST_API_URL}/todolist-reset`, { method: "post" }).then(
      () => {
        this.getItems();
      }
    );
  ...
  getItems = () => {
    return fetch(`${TODO_LIST_API_URL}/todolist`, {
      method: "get"
    })
      .then(res => {
        return res.json();
  ...
  deleteItem = id => {
    return fetch(`${TODO_LIST_API_URL}/todolist/${id}`, {
      method: "delete"
    }).then(res => {
      return res.text();
    });
  ...
  addItem = text => {
    return fetch(`${TODO_LIST_API_URL}/todolist/${text}`, {
      method: "post"
    }).then(res => {
      return res.text();
    });
            ...
            {this.state.items.map(item => {
              return (
                <li key={item.id} style={{ display: "block" }}>
                  <span data-hook={`todolist__item-text-${item.id}`}>
                    {item.text}{" "}
                  </span>
                  <button
